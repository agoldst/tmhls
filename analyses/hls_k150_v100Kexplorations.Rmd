...
```{r}
cite_articles(m$metadata,top_documents(16,m$id_map,dtm=m$dtm,n=10)$id)
```
...
```{r}
word_topic_breakdown <- function(words,geom="area",verbose=T) { 
  ...
  if(geom=="line") {
    ...
    stop("I don't know what to do with the geom ",geom)
  }
  result
}
```
